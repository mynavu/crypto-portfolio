"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.DISCRIMINATOR = void 0;
exports.initializeFarmDelegated = initializeFarmDelegated;
const programId_1 = require("../programId");
exports.DISCRIMINATOR = Buffer.from([250, 84, 101, 25, 51, 77, 204, 91]);
function initializeFarmDelegated(accounts, remainingAccounts = [], programAddress = programId_1.PROGRAM_ID) {
    const keys = [
        {
            address: accounts.farmAdmin.address,
            role: 3,
            signer: accounts.farmAdmin,
        },
        {
            address: accounts.farmDelegate.address,
            role: 2,
            signer: accounts.farmDelegate,
        },
        { address: accounts.farmState, role: 1 },
        { address: accounts.globalConfig, role: 0 },
        { address: accounts.farmVaultsAuthority, role: 0 },
        { address: accounts.systemProgram, role: 0 },
        { address: accounts.rent, role: 0 },
        ...remainingAccounts,
    ];
    const data = exports.DISCRIMINATOR;
    const ix = { accounts: keys, programAddress, data };
    return ix;
}
//# sourceMappingURL=initializeFarmDelegated.js.map