...
class Farms {
    ...
    async getAllUserStatesForUser(user) {
        ...
        filters.push({ dataSize: BigInt(accounts_2.UserState.layout.span + 8) });
        return (await this._connection
            ...
            .send()).map((x) => {
            const userAndKey = {
                userState: accounts_2.UserState.decode(Buffer.from(x.account.data[0], "base64")),
                key: x.pubkey,
            };
            return userAndKey;
        });
    ...
    async getAllUserStates() {
        return (await this._connection
            .getProgramAccounts(this._farmsProgramId, {
            filters: [{ dataSize: BigInt(accounts_2.UserState.layout.span + 8) }],
            encoding: "base64",
        })
            .send()).map((x) => {
            const userAndKey = {
    ...
    async getAllUserStatesWithFilter(isFarmDelegated) {
        return (await this._connection
            .getProgramAccounts(this._farmsProgramId, {
            filters: [
                { dataSize: BigInt(accounts_2.UserState.layout.span + 8) },
                {
                    memcmp: {
                        offset: 80n,
    ...
    async getAllUserStatesForFarm(farm) {
        return (await this._connection
            ...
            const userAndKey = {
                userState: accounts_2.UserState.decode(Buffer.from(decompressedData)),
                key: x.pubkey,
            };
            return userAndKey;
        });
    ...
    async getFarmsForMint(mint) {
        ...
        filters.push({ dataSize: BigInt(accounts_2.FarmState.layout.span + 8) });
        return (await this._connection
            .getProgramAccounts(this._farmsProgramId, {
            filters,
            encoding: "base64",
            ...
            .send()).map((x) => {
            const farmAndKey = {
                farmState: accounts_2.FarmState.decode(Buffer.from(x.account.data[0], "base64")),
                key: x.pubkey,
            };
            return farmAndKey;
        });
    ...
    async getAllFarmStates() {
        return (await this._connection
            .getProgramAccounts(this._farmsProgramId, {
            filters: [{ dataSize: BigInt(accounts_2.FarmState.layout.span + 8) }],
            encoding: "base64",
        })
            .send())
            .map((x) => {
    ...
    async getLockupDurationAndExpiry(farm, user, timestampNow) {
        ...
        let userState = await accounts_2.UserState.fetch(this._connection, userStateAddress);
        let farmState = await accounts_2.FarmState.fetch(this._connection, farm);
        if (!farmState) {
    ...
    async getUserStateKeysForDelegatedFarmDeterministic(user, farm, delegatees) {
        ...
        const userStates = await accounts_2.UserState.fetchMultiple(this._connection, userStateAddresses);
    ...
    async getUserStateKeyForUndelegatedFarm(user, farmAddress) {
        ...
        const userState = await accounts_2.UserState.fetch(this._connection, userStateAddress);
        if (!userState) {
    ...
    async getUserForUndelegatedFarm(user, farmAddress, timestamp) {
        const farmState = await accounts_2.FarmState.fetch(this._connection, farmAddress);
        if (!farmState) {
    ...
    async claimForUserForFarmAllRewardsIx(payer, user, farm, isDelegated, delegatees) {
        ...
        for (let userStateIndex = 0; userStateIndex < userStatesAndKeys.length; userStateIndex++) {
            for (let rewardIndex = 0; rewardIndex < farmState.numRewardTokens.toNumber(); rewardIndex++) {
                ...
                const rewardMinClaimDurationSeconds = farmState.rewardInfos[rewardIndex].minClaimDurationSeconds.toNumber();
                const lastClaimTsSeconds = userStatesAndKeys[userStateIndex].userState.lastClaimTs[rewardIndex].toNumber();
    ...
    async transferOwnershipIx(user, userState, newUser) {
        const userStateData = await accounts_2.UserState.fetch(this._connection, userState, this._farmsProgramId);
        if (!userStateData) {
            throw new Error(`User state not found ${userState.toString()}`);
        }
        const farmState = await accounts_2.FarmState.fetch(this._connection, userStateData.farmState, this._farmsProgramId);
        if (!farmState) {
    ...
    async rewardUserOnceIx(delegateAuthority, farmState, userState, rewardMint, amountLamports, expectedRewardIssuedUnclaimed) {
        const rewardIndex = farmState.farmState.rewardInfos.findIndex((r) => r.token.mint === rewardMint);
        const ix = farmOperations.rewardUserOnce(delegateAuthority, farmState.key, userState, rewardIndex, amountLamports, expectedRewardIssuedUnclaimed);
        return ix;
    }
    ...
    async updateFarmRpsForRewardIx(payer, rewardMint, farm, rewardsPerSecond) {
        ...
        const farmState = await accounts_2.FarmState.fetch(this._connection, farm, farmsClient.getProgramID());
        if (!farmState) {
    ...
    async topUpFarmForRewardIx(payer, rewardMint, farm, amountToTopUp) {
        const farmState = await accounts_2.FarmState.fetch(this._connection, farm, this.getProgramID());
        if (!farmState) {
            throw new Error("Farm not found");
        }
        let rewardIndex = 0;
    ...
    async fetchMultipleFarmStatesWithCheckedSize(keys) {
        ...
        const farmStateSize = BigInt(accounts_2.FarmState.layout.span + 8);
        ...
        return infos.value.map((info) => {
            ...
            return accounts_2.FarmState.decode(Buffer.from(info.data[0], "base64"));
        });
    }
