...
class MarginfiClient {
    ...
    static async fetchGroupData(program, groupAddress, commitment, bankAddresses, bankMetadataMap) {
        ...
        if (bankAddresses && bankAddresses.length > 0) {
            ...
            bankDatasKeyed = bankAccountsData
                .filter((data) => data.account.config.assetTag < services_1.AssetTag.KAMINO)
                .map((account) => ({
        ...
        const priceInfos = new Map(bankDatasKeyed.map(({ address: bankAddress, data: bankData }, index) => {
            ...
            const fixedPrice = (0, mrgn_common_1.wrappedI80F48toBigNumber)(bankData.config.fixedPrice);
            return [bankAddress.toBase58(), (0, __1.parsePriceInfo)(oracleSetup, priceDataRaw.data, fixedPrice)];
        }));
    ...
    getBankByMint(mint) {
        ...
        return ([...this.banks.values()].find((bank) => bank.mint.equals(_mint) &&
            (bank.config.assetTag === services_1.AssetTag.DEFAULT ||
                bank.config.assetTag === services_1.AssetTag.SOL ||
                bank.config.assetTag === services_1.AssetTag.STAKED)) ?? null);
    }
    getBankByTokenSymbol(tokenSymbol) {
        if (tokenSymbol === undefined)
            return null;
        return ([...this.banks.values()].find((bank) => bank.tokenSymbol === tokenSymbol &&
            (bank.config.assetTag === services_1.AssetTag.DEFAULT ||
                bank.config.assetTag === services_1.AssetTag.SOL ||
                bank.config.assetTag === services_1.AssetTag.STAKED)) ?? null);
    }
