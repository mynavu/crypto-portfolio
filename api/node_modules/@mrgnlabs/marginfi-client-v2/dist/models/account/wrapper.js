...
class MarginfiAccountWrapper {
    ...
    async simulateBorrowLendTransaction(txs, banksToInspect, healthSimOptions) {
        ...
        if (healthSimOptions?.enabled) {
            ...
            const updateFeedTx = new web3_js_1.VersionedTransaction(new web3_js_1.TransactionMessage({
                instructions: [...updateFeedIx.instructions],
                payerKey: this.client.provider.publicKey,
                recentBlockhash: blockhash,
            }).compileToV0Message([...updateFeedIx.luts]));
            const healthCacheTx = new web3_js_1.VersionedTransaction(new web3_js_1.TransactionMessage({
    ...
    async makeWithdrawStakedTx(amount, bankAddress, isWholePosition) {
        ...
        const { instructions: updateFeedIxs, luts: feedLuts } = await this.makeUpdateFeedIx(solBank?.address ? [solBank.address] : []);
        ...
        // build transaction
        const { value: { blockhash }, } = await this._program.provider.connection.getLatestBlockhashAndContext("confirmed");
        const crankMessage = new web3_js_1.TransactionMessage({
            payerKey: this.client.wallet.publicKey,
            recentBlockhash: blockhash,
            instructions: updateFeedIxs,
        }).compileToV0Message(feedLuts);
        const crankTxn = (0, mrgn_common_1.addTransactionMetadata)(new web3_js_1.VersionedTransaction(crankMessage), {
            addressLookupTables: feedLuts,
            type: mrgn_common_1.TransactionType.CRANK,
        });
     ...
     */
    async makeBeginFlashLoanIx(endIndex, authority) {
     ...
     */
    async makeEndFlashLoanIx(projectedActiveBalances, authority) {
        return this._marginfiAccount.makeEndFlashLoanIx(this._program, this.client.banks, projectedActiveBalances, authority);
    }
    ...
    async buildFlashLoanTx(args, lookupTables, authority) {
    ...
    async makeAccountTransferToNewAccountIx(newMarginfiAccount, newAccountAuthority, globalFeeWallet, feePayer) {
        return this._marginfiAccount.makeAccountTransferToNewAccountIx(this._program, newMarginfiAccount, newAccountAuthority, globalFeeWallet, feePayer);
    }
    async makeAccountTransferToNewAccountTx(newMarginfiAccount, newAccountAuthority, feePayer) {
        ...
        const ixs = await this.makeAccountTransferToNewAccountIx(newMarginfiAccount, newAccountAuthority, feeState.globalFeeWallet, feePayer);
        const tx = new web3_js_1.Transaction().add(...ixs.instructions);
        return tx;
    }
    async makeUpdateFeedIx(newBanksPk, txLandingBuffer = 0) {
        ...
        if (swbPullBanks.length > 0) {
            const filteredSwbPullBanks = swbPullBanks
                ...
                if (activeBalance?.assetShares.gt(new bignumber_js_1.default(0)) && bank.config.riskTier === __1.RiskTier.Isolated) {
                    return false;
                }
                return true;
            })
                .filter((bank) => {
                // filter 0 feeds
                return !bank.oracleKey.equals(new web3_js_1.PublicKey("DMhGWtLAKE5d56WdyHQxqeFncwUeqMEnuC2RvvZfbuur"));
            })
            ...
            if (filteredSwbPullBanks.length > 0) {
                ...
                const pullFeedInstances = filteredSwbPullBanks.map((pubkey) => new on_demand_1.PullFeed(swbProgram, pubkey));
                const crossbarClient = new common_1.CrossbarClient(process.env.NEXT_PUBLIC_SWITCHBOARD_CROSSSBAR_API || "https://34.97.218.183.sslip.io");
