...
function computeEmodeImpacts(emodePairs, activeLiabilities, activeCollateral, allBanks) {
    ...
    function simulate(bank, action) {
        const isSolBank = bank.equals(new web3_js_1.PublicKey("CCKtUs6Cgwo4aaQUmBPmyoApH2gUDErxNZCAntD6LYGh"));
        ...
        const activeEmodePair = bestPair
            ? {
                collateralBanks: Array.from(new Map(after
                    .map((p) => p.collateralBanks)
                    .flat()
                    .map((bank) => [bank.toBase58(), bank])).values()),
                collateralBankTags: Array.from(new Set(after.map((p) => p.collateralBankTag).flat())),
                liabilityBanks: Array.from(new Map(after
