...
exports.adjustPricesStakedCollateral = exports.fetchOracleData = exports.mapSwbBanksToOraclePrices = exports.fetchSwbOracleData = exports.fetchPythOracleData = exports.extractPythOracleKeys = exports.categorizePythBanks = void 0;
...
const categorizePythBanks = (banks) => {
    // Depracated
    const pythLegacyBanks = banks.filter((bank) => bank.data.config.oracleSetup && "pythLegacy" in bank.data.config.oracleSetup);
    // Pyth push oracle banks
    const pythPushBanks = banks.filter((bank) => bank.data.config.oracleSetup && "pythPushOracle" in bank.data.config.oracleSetup);
    ...
    const pythPushIntegrationBanks = banks.filter((bank) => bank.data.config.oracleSetup &&
...
exports.categorizePythBanks = categorizePythBanks;
/**
 ...
 */
const extractPythOracleKeys = (pythBanks) => {
 ...
 */
const mapPythBanksToOraclePrices = (pythPushBanks, oraclePrices) => {
...
const fetchPythOracleData = async (banks, bankMetadataMap, connection, opts) => {
    ...
    const { pythPushBanks, pythStakedCollateralBanks, pythPushIntegrationBanks } = (0, exports.categorizePythBanks)(banks);
    ...
    const pythOracleKeys = (0, exports.extractPythOracleKeys)(combinedPythBanks);
