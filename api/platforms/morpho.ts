...
import {USDC_str, USDT_str, WBTC_str, WSOL_str, WETH_str} from '../address.ts'
import {
  createPublicClient,
  http,
  parseAbi,
...
export async function getMorphoAPY(
  marketIds: `0x${string}`[], token_address: any
) {
  const results: {
    marketId: string;
    borrowAPY: number;
...
*/
    ...
    res.status(200).json({usdcSupplyAPY, usdcBorrowAPY, usdtSupplyAPY, usdtBorrowAPY, btcSupplyAPY, btcBorrowAPY, ethSupplyAPY, ethBorrowAPY})
  } catch (err) {
    console.error(err)
